...
public class CameraControllerwithBuffer : MonoBehaviour
{
   ...
   [Range(1.0f, 10.0f)] [SerializeField] private float cameraOffsetY = 5.0f;
    ...
    void Update()
    {
        ...
        if(player.position.x < transform.position.x - (0.5f * cameraOffsetX)) // left side
        {
            transform.position = new Vector3(player.position.x + (0.5f * cameraOffsetX), transform.position.y, transform.position.z);
        }
        ...
        if (player.position.y < transform.position.y - (0.5f * cameraOffsetY)) // left side
        {
            transform.position = new Vector3(transform.position.x, player.position.y + (0.5f * cameraOffsetY), transform.position.z);
        }
